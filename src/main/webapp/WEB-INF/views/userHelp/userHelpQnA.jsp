...
<body>
<div id="contents" style="height:350px; background:white; margin:0 auto; margin-bottom:300px; border:0px solid black; position:static;">
			...
			<div class="point_body" style="width:400px;margin-left:110px">
				...
				<div align="center" style="width:350px">
					...
					<c:if test="${startPage>pageBlock}">
						<a href="${root}/userHelp/userHelpQnA.do?pageNumber=${startPage-pageBlock}">[이전]</a>
					</c:if>
					
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<a href="${root}/userHelp/userHelpQnA.do?pageNumber=${i}">[${i}]</a>
					</c:forEach>
					
					<c:if test="${endPage<pageCount}">
						<a href="${root}/userHelp/userHelpQnA.do?pageNumber=${startPage+pageBlock}">[다음]</a>
					</c:if>
				</div>
			</div>
		</div>
