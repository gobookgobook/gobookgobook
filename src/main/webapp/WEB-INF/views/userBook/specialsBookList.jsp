...
<div id="contents" style="height:800px; background:white; border:0px solid black; position:static;">
	...
	<div align="left" style="width: 120px; float:left;">
		...
		<div class="userBookBody">
			<c:if test="${specialsBookCount > 0 }">
				<c:forEach var="userBook" items="${specialsBook}">
		...
		<div align="center" style="border:0px solid green; margin-top:720px; margin-left:430px; width:200px; height:20px;">
			<c:if test="${specialsBookCount > 0}">
				...
				<c:set var="pageCount" value="${specialsBookCount/boardSize+(specialsBookCount%boardSize==0 ? 0:1)}"/>
				<fmt:parseNumber var="pageCount" value="${specialsBookCount/boardSize+(specialsBookCount%boardSize==0 ? 0:1)}" integerOnly="true"/>
			...
			<c:if test="${startPage > pageBlock }">
				<a href="${root}/userBook/userBookListSpecials.do?pageNumber=${startPage-1}">[이전]</a>
			</c:if> 
	 			
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="${root}/userBook/userBookListSpecials.do?pageNumber=${i}">[${i}]</a>
			</c:forEach>
			
			<c:if test="${endPage < pageCount}">
				<a href="${root}/userBook/userBookListSpecials.do?pageNumber=${startPage+pageBlock}">[다음]</a>
			</c:if>  
		</div>
</div>
