							...
							<tr class="info" id="${myBasket.basket_num}">
								...
								<td>
									<form name="quantity_update" method="post">
										<input type="text" name="upQuantity" value="${myBasket.basket_quantity}" size="1" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)' style='ime-mode:disabled;'/>&nbsp;
										<c:set var="value" value="upQuantity.value"/>
										<input type="button" class="btn btn-info btn-xs" value="수정" onclick="updateToServer('${myBasket.basket_num}', '${myBasket.basket_book_price}', ${value}, '${sum}', '${root}')"/>
									</form>
								</td>
								...
								<td>
									<input type="button" id="delete" class="btn btn-warning btn-xs" value="삭제" onclick="javascript:deleteToServer('${myBasket.basket_num}','${myBasket.basket_total_price}','${sum}','${root}')"/>
								</td>
							</tr>
						</c:forEach>
